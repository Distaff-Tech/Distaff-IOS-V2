...
extension SignInVC:GIDSignInDelegate {
    func sign(_ signIn: GIDSignIn!, didSignInFor user: GIDGoogleUser!, withError error: Error!) {
        if (error == nil) {
            ...
            objSIgnInVM.callSocialSignInApi(info: SocialInfo_User(email: email, socialId: userId, userName: name, fullName: fullName, profilePic: userProfile,loginType: LogedInUserType.gmail))
...
extension SignInVC : ASAuthorizationControllerDelegate {
    ...
    func authorizationController(controller: ASAuthorizationController, didCompleteWithAuthorization authorization: ASAuthorization) {
        ...
        if let appleIDCredential = authorization.credential as? ASAuthorizationAppleIDCredential {
            objSIgnInVM.fetchAppleLoginData(appleIDCredential: appleIDCredential)
        }
    ...
    func authorizationController(controller: ASAuthorizationController, didCompleteWithError error: Error) {
        print(error)
      //  self.showAlert(message: Messages.LocalFailureMessages.cancelledFlow)
    }
}
