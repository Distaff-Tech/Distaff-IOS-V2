...
extension PostDetailVC : UICollectionViewDelegate,UICollectionViewDataSource,UICollectionViewDelegateFlowLayout {
    ...
    func collectionView(_ collectionView: UICollectionView, numberOfItemsInSection section: Int) -> Int {
        return (collectionView == collectionViewSize ? (objVMPostDetail.postDetailData?.size?.count ?? 0) : collectionView == collectionViewColor ? (objVMPostDetail.postDetailData?.post_image?.count ?? 0) :objVMPostDetail.postDetailData?.post_image?.count ?? 0)
    }
