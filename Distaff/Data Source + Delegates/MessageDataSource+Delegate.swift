    ...
    func tableView(_ tableView: UITableView, trailingSwipeActionsConfigurationForRowAt indexPath: IndexPath) -> UISwipeActionsConfiguration? {
        ...
        let action =  UIContextualAction(style: .normal, title:nil, handler: { (action,view,completionHandler ) in
            ...
            self.showAlertWithCancelAndOkAction(message: "\(Messages.DialogMessages.deleteEntireChat)\(" ")\(userName)", onComplete: {
                ...
                let request = DeleteChat.Request(receiverId: (userdefaultsRef.value(Response.self, forKey: UserDefaultsKeys.userInfo))?.id == self.objVMMessage.recentMessageList[indexPath.row].sender_id ?? 0 ? self.objVMMessage.recentMessageList[indexPath.row].receiver_id ?? 0 :  self.objVMMessage.recentMessageList[indexPath.row].sender_id ?? 0, currentPage: self.objVMMessage.pageNumber)
                ...
                self.objVMMessage.callDeleteChatApi(request) { (data) in
                    ...
                    if data.data?.count != 0 {
                        ...
                        self.objVMMessage.recentMessageList.append(objectToAppend!)
                    }
