    ...
    func tableView(_ tableView: UITableView, trailingSwipeActionsConfigurationForRowAt indexPath: IndexPath) -> UISwipeActionsConfiguration? {
        let action =  UIContextualAction(style: .normal, title:nil, handler: { (action,view,completionHandler ) in
            ...
            self.showAlertWithCancelAndOkAction(message: Messages.DialogMessages.deleteOrder, onComplete: {
                ...
                let request = DeleteOrder.Request(orderId: isMyRequest == true ? self.objMyOrderVM.myRequestArray[indexPath.section]?.list?[indexPath.row].id : self.objMyOrderVM.myOrdersArray[indexPath.section]?.list?[indexPath.row].id, timeZone: localTimeZoneName, type: isMyRequest ? "my_request" : "my_past",pageNo: isMyRequest ? self.objMyOrderVM.myRequestPageNumber :self.objMyOrderVM.myOrdersPageNumber )
                
                self.objMyOrderVM.callDeleteOrderApi(request: request) { (data) in
                    if !isMyRequest {
                        ...
                        self.appendNextItemOnDeletion(data: data , isMyRequest: false)
                        ...
                        self.appendNextItemOnDeletion(data: data, isMyRequest: true)
