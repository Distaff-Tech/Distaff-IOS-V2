    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell?.callBackLike = {
            CommonMethods.callLikeUnlikeApi(postId: self.objFavouriteVM.postListArray[indexPath.row].post ?? 0, status: !(self.objFavouriteVM.postListArray[indexPath.row].post_like ?? false)) {
                ...
                cell?.btnLike.isSelected = self.objFavouriteVM.postListArray[indexPath.row].post_like ?? false ? true : false
               cell?.lblLikeCount.text = String( self.objFavouriteVM.postListArray[indexPath.row].total_likes ?? 0)
