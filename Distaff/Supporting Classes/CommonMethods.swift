...
class CommonMethods: NSObject {
    ...
    class func callLikeUnlikeApi(postId:Int,status:Bool,_completion:@escaping() -> Void) {
        Services.postRequest(url: WebServicesApi.like_dislike_post, param: [like_UnlikePost.post_id:postId,like_UnlikePost.like_status:status], shouldAnimateHudd: false) { (responseData) in
            do {
                let data = try JSONDecoder().decode(SignInModel.self, from: responseData)
                print(data)
                _completion()
    ...
    class func callFavouriteUnfavouriteApi(postId:Int,status:Bool,_completion:@escaping() -> Void) {
        Services.postRequest(url: WebServicesApi.set_favourite, param: [fav_UnfavouritePost.post_id:postId,fav_UnfavouritePost.fav_status:status], shouldAnimateHudd: false) { (responseData) in
            do {
                let data = try JSONDecoder().decode(SignInModel.self, from: responseData)
                print(data)
                _completion()
