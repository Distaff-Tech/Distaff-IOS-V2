...
extension UIViewController {
    ...
    func likeAndSavedCollectionAnimation(btn:UIButton) {
        UIView.animate(withDuration: 0.2, animations: {
            btn.transform = CGAffineTransform.init(scaleX: 1.35, y: 1.35)
        }) { (true) in
    ...
    func isGuestUser() -> Bool {
        return ((userdefaultsRef.value(forKey: UserDefaultsKeys.isUserLogedIn)) == nil)
    }
    
    
    func displayGuestUserAlert() {
        let alert = UIAlertController(title: AppInfo.appName, message:Messages.DialogMessages.loginFirst, preferredStyle: .actionSheet)
    ...
    func getViewWithIdentifer<T: UIView>(ofType type: T.Type,identifer:String,view:UIView) -> T?  {
        var viewToReturn: T?
        for i in view.subviewsRecursive() {
            if i.restorationIdentifier == identifer {
                viewToReturn = i as? T
                break
            }
...
extension UINavigationBar {
    ...
    func lt_setBackgroundColor(backgroundColor: UIColor) {
        if overlay == nil {
            ...
            overlay = UIView.init(frame: CGRect.init(x: 0, y: 0, width: bounds.width, height: bounds.height+statusBarHeight))
