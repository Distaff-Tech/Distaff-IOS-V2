...
class CreateProfileVC: BaseClass {
    ...
    override func scrollViewDidScroll(_ scrollView: UIScrollView) {
        ...
        let colorToSet = AppColors.appColorBlue
        if (offsetY > 35) {
            let alpha = min(1, 1 - ((35 + 64 - offsetY) / 64));
            self.navigationController?.navigationBar.lt_setBackgroundColor(backgroundColor:colorToSet.withAlphaComponent(alpha))
            
        } else {
            self.navigationController?.navigationBar.lt_setBackgroundColor(backgroundColor: colorToSet.withAlphaComponent(0))
...
extension CreateProfileVC {
    ...
    func displayUsersSocialData(info:SocialInfo_User) {
        ...
        if info.loginType == LogedInUserType.apple  {
            txtFldFullName.isHidden = true
...
extension CreateProfileVC {
    @IBAction func didTappedNext(_ sender: UIButton) {
        dismissKeyboard()
        let request = CreateProfile.Request(userName: txtFldUserName.text, fullName: txtFldFullName.text, completeAddress: txtDldAddress.text,profilePicture:objCreateProfileVM.capturedImage, isFirstPage: true,isFromEditProfile:isFromEditProfile ? 1:0,profileData:profileData)
