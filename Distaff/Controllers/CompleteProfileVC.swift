...
extension CompleteProfileVC {
    func prepareUI() {
        ...
        showNavigationBar()
        txtViewAbout.setPlaceholder(with: "Enter Description (Optional)", padding: UIEdgeInsets(top: 10, left: 10, bottom: 0, right: 20), placeholderColor:#colorLiteral(red: 0.7803921569, green: 0.7803921569, blue: 0.8039215686, alpha: 1))
        ...
        if profileData != nil {
            ...
            btnOther.isSelected  = profileData?.data?.gender ?? "" == "O"
            if let dob =  profileData?.data?.date_of_birth  {
                txtFldDate.text =  CommonMethods.convertDateFormat(inputFormat: "yyyy-MM-dd", outputFormat: "MM-dd-yyyy", dateString: dob)
            }
...
extension CompleteProfileVC {
    @IBAction func didTappedGenderButtons(_ sender: UIButton) {
        ...
        btnOther.isSelected = sender == btnOther ? true :false
    ...
    @IBAction func didTappedSubmit(_ sender: UIButton) {
        dismissKeyboard()
        let request = CreateProfile.Request(userName: profileInfo?.userName, fullName: profileInfo?.fullName, completeAddress: profileInfo?.completeAddress,profilePicture:profileInfo?.profilePicture ,gender: (!btnMale.isSelected && !btnFemale.isSelected && !btnOther.isSelected) ? "": btnMale.isSelected ? "M": btnFemale.isSelected ? "F" : "O", DateOfBirth: txtFldDate.text, about: txtViewAbout.text, isFirstPage: false,isFromEditProfile:profileData != nil ? 1 : 0)
        objCreateProfileVM.callCreateProfileApi(request) { (data) in
            ...
            userinfo?.canAddPost = data?.data?.canAddPost
