...
class PostDetailVC: BaseClass {
    ...
    @IBOutlet weak var lblFrontPrintingSize: UILabel!
    @IBOutlet weak var lblBackPrintingSize: UILabel!
    @IBOutlet weak var printingAreaHeightAnchor: NSLayoutConstraint!
    @IBOutlet weak var clothToAnchor: NSLayoutConstraint!
...
extension PostDetailVC {
    @IBAction func didTappedcartAction(_ sender: UIButton) {
        isGuestUser() ? displayGuestUserAlert() : handleCartButtonSelection(btnCart: btnCart)
    ...
    @IBAction func didTappedMessage(_ sender: UIButton) {
        ...
        isGuestUser() ? displayGuestUserAlert() : self.navigationController?.pushViewController(targetVC ?? UIViewController(), animated: true)
    }
    
    @IBAction func didTappedLike(_ sender: UIButton) {
        isGuestUser() ? displayGuestUserAlert() : CommonMethods.callLikeUnlikeApi(postId: postId ?? 0, status: btnLike.isSelected == true ? false : true) {
            ...
            self.likeAndSavedCollectionAnimation(btn: sender)
        }
    ...
    @IBAction func didTappedSavedInCollection(_ sender: UIButton) {
        isGuestUser() ? displayGuestUserAlert() : CommonMethods.callFavouriteUnfavouriteApi(postId: postId ?? 0, status: btnSaveToCollection.isSelected == true ? false : true) {
    ...
    @IBAction func didTappedOptionMenu(_ sender: UIButton) {
        isGuestUser() ? displayGuestUserAlert() : CommonMethods.reportPostOptions { (text) in
