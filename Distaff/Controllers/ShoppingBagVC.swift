    ...
    extension ShoppingBagVC {
        ...
        @IBAction func didTappedContinue(_ sender: UIButton) {
            ...
            targetVc?.shippingCharge = Double(calculateTotalPrice().2)
            self.navigationController?.pushViewController(targetVc ?? UIViewController(), animated: true)
        }
    ...
    extension ShoppingBagVC {
        ...
        func calculateTotalPrice()  -> (String,String,String) {
            ...
            let shippingPrice = (objShoppingBagVM.shippingCharge ?? 0.0) * Double(objShoppingBagVM.shoppingBagItems.count)
            let totalPrice = subTotalPrice + servicePrice + shippingPrice
            return (totalPrice.calculateCurrency(fractionDigits: 2),servicePrice.calculateCurrency(fractionDigits: 2),shippingPrice.calculateCurrency(fractionDigits: 2))
        }
