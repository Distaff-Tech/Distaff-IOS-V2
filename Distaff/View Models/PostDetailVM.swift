...
class PostDetailVM {
    ...
    func callPostDetailApi(view:UIView,url:String,completion:@escaping(_ data:PostData?) -> Void) {
    ...
    func callAddToCartApi(_ request: AddToCart.Request,completion:@escaping(_ data:AddToCartModel?) -> Void) {
    ...
    func displayPostdetailData(ref:PostDetailVC,data:PostData?) {
        ...
        ref.lblAddress.text = data?.address ?? "" == "" ? "Not added yet" : data?.address ?? ""
        ...
        // printing Area
        ref.printingAreaHeightAnchor.constant = data?.post_printing_size_front ?? "" == "" ? 0 : 63.5
        ref.lblFrontPrintingSize.text = "Front Size: \(data?.post_printing_size_front ?? "")"
        ref.lblBackPrintingSize.text = "Back Size: \(data?.post_printing_size_back ?? "")"
        ref.clothToAnchor.constant = data?.post_printing_size_front ?? "" == "" ? 0 : 15
        ref.btnSaveToCollection.isHidden = ref.btnAddToCart.isHidden
    }
