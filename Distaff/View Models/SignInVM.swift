...
class SignInVM {
    ...
    func loginWithApple(ref:UIViewController) {
        if #available(iOS 13.0, *) {
            ...
            authorizationController.delegate = ref as? ASAuthorizationControllerDelegate
            authorizationController.presentationContextProvider = ref as? ASAuthorizationControllerPresentationContextProviding //
            ...
            .responseJSON { response in
                ...
                case .success (_):
                    ...
                    if let dataDict = response.result.value as? NSDictionary {
                        ...
                        //                        }
                        socialInfo.loginType = LogedInUserType.instagram
    ...
    func callSocialSignInApi(info:SocialInfo_User?) {
        ...
        Services.postRequest(url: WebServicesApi.socialLogin, param: [Social_SignIN.email: info?.email ?? "",Social_SignIN.social_id: info?.socialId ?? "",Social_SignIN.deviceId : userdefaultsRef.value(forKey: UserDefaultsKeys.deviceId) as? String ?? "No Token",Social_SignIN.deviceType : AppInfo.deveiceType,Social_SignIN.login_type:info?.loginType?.rawValue ?? ""], shouldAnimateHudd: true) { (responseData) in
            do {
    ...
    @available(iOS 13.0, *)
    func fetchAppleLoginData(appleIDCredential:ASAuthorizationAppleIDCredential) {
        ...
        let savedId = objKeychain.get(Apple_Info.userId) ?? ""
        let email = appleIDCredential.email ?? objKeychain.get(Apple_Info.email) ?? ""
        let firstName = appleIDCredential.fullName?.givenName ?? objKeychain.get(Apple_Info.firstName) ?? " "
        let lastName = appleIDCredential.fullName?.familyName ?? objKeychain.get(Apple_Info.lastName) ?? " "
        
        
        if savedId == "" || savedId == userId {
            objKeychain.set(userId, forKey: Apple_Info.userId)
            objKeychain.set(firstName, forKey: Apple_Info.firstName)
            objKeychain.set(lastName, forKey: Apple_Info.lastName)
            objKeychain.set(email, forKey: Apple_Info.email)
            
            self.callSocialSignInApi(info: SocialInfo_User(email: objKeychain.get(Apple_Info.email), socialId: objKeychain.get(Apple_Info.userId), fullName: "\(objKeychain.get(Apple_Info.firstName) ?? "") \(objKeychain.get(Apple_Info.lastName) ?? "")",loginType:LogedInUserType.apple))
        ...
        if((AccessToken.current) != nil){
            GraphRequest(graphPath: "me", parameters: ["fields": "id, name, first_name, last_name, picture.type(large), email"]).start(completionHandler: { (connection, result, error) -> Void in
                ...
                if (error == nil){
                    ...
                    userProfileInfo.loginType = LogedInUserType.facebook
