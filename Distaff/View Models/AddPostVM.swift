...
class AddPostVM{
    ...
    var printingSizeFront:String?
    var printingSizeBack:String?
    ...
    func callAddPostApi(_ request: AddPost.Request,_completion:@escaping(_ data:CreateProfileModel?) -> Void) {
        if formValidations(request) {
            ...
            let dict = [Add_Post.price:request.price ?? "0",Add_Post.postDescription:request.description ?? "",Add_Post.fabric:"2",Add_Post.size:request.sizesAvailable ?? "",Add_Post.post_type : isFromCustomDesign ?  "custom" : ""] as [String : Any]
            
            
            var param = [Add_Post.data:CommonMethods.convertDictToJsonString(dictionary: dict)]
            print(param)
            
            if isFromCustomDesign {
                param[Add_Post.printingSizeFront] = printingSizeFront
                param[Add_Post.printingSizeBack] = printingSizeBack
            }
